...
class OutNode extends AbstractTypeListNode {
    ...
    void genParseCommandPacket(PrintWriter writer, int depth) {
        writer.println();
        indent(writer, depth);
        writer.println(
                "static " + cmdName + " parseCommandPacket(VM vm, Packet packet){");
        indent(writer, depth+1);
        writer.println(
                "PacketStream ps = new PacketStream(vm, packet);");
        indent(writer, depth+1);
    ...
    void genGenerateCommandPacket(PrintWriter writer, int depth) {
        writer.println();
        indent(writer, depth);
        writer.println(
                "void Packet generateCommandPacket(VM vm){");
        indent(writer, depth+1);
        writer.println(
                "PacketStream ps = new PacketStream(vm, COMMAND_SET, COMMAND);");
        genJavaWrites(writer, depth+1);
        indent(writer, depth+1);
        writer.println("return ps.toPacket();");
        indent(writer, depth);
        writer.println("}");
    }

    void genParseReturnPacket(PrintWriter write, int depth) {

    }

    void getGenerateReturnPacket(PrintWriter write, int depth) {
